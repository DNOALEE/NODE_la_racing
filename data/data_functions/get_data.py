...
from data.data_functions.utils import first_index, array_to_tensor
from data.data_functions.change_of_coords_frame import cart_to_fren, fren_to_cart, frame_rotaton


def ros_data(index_0,sim):
    base_dir = os.path.dirname(os.path.realpath(__file__))
    if sim:
        car_set_control = os.path.join(base_dir, '..', 'data_files', 'sim_car', 'sim_car_set_control.csv')
        # car_set_control = r'\\wsl.localhost\Ubuntu-20.04\home\oliver\ros2_ws\ros_bag\simcar2\simcar2_0\sim_car_set_control.csv'
        car_state = os.path.join(base_dir, '..', 'data_files', 'sim_car', 'sim_car_state.csv')
        # car_state = r'\\wsl.localhost\Ubuntu-20.04\home\oliver\ros2_ws\ros_bag\simcar2\simcar2_0\sim_car_state.csv'
    else:
        car_set_control = os.path.join(base_dir, '..', 'data_files', 'car', 'car_set_control.csv')
        ...
        car_state = os.path.join(base_dir, '..', 'data_files', 'car', 'car_state.csv')
        # car_state = r'C:\Users\Oliver Ljungberg\Desktop\EPFL\MA3\Semester_Project\experiment_car_1_track_manual_var_speed\car_state.csv'

    data = pd.read_csv(car_state)
    x = data.pos_x
    y = data.pos_y
...
def track_data(sim,transform=True):
    base_dir = os.path.dirname(os.path.realpath(__file__))
    la_track = os.path.join(base_dir, '..', 'data_files', 'la_track.yaml')
    # la_track = r'C:\Users\Oliver Ljungberg\Desktop\EPFL\MA3\Semester_Project\Code\data\data_files\la_track.yaml'
